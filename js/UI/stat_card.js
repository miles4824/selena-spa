...
function StatCard({
  ...
  const resolvedEyeId = privacyEyeId || (id ? `${id}-eye-icon` : 'stat-card-eye-icon');
  const resolvedEyeBtnId = id ? `${id}-eye-btn` : 'stat-card-eye-btn';
  const resolvedToggleAction = onPrivacyToggle || (id ? `StatCard.toggle('${id}', '${privacyType}')` : '');
...
StatCard.toggle = function(cardId, privacyType) {
  try {
    ...
    if (privacyType === 'staff_comm') {
      ...
      const stats = (typeof currentUser !== 'undefined') ? HomeService.getStaffTodayStats(currentUser) : null;
      newValue = isMasked ? '•••••• đ' : (stats ? stats.formattedCommission : '0 đ');
    } else if (privacyType === 'owner_revenue') {
    ...
    if (eyeBtn) {
      ...
      const eyeEl = document.getElementById(eyeId) || (valEl ? valEl.parentElement.querySelector('[data-lucide]') : null);
      if (eyeEl) {
