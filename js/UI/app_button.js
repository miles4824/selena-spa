...
function AppButton({
  ...
  iconColor = "", // Tùy chọn chỉ định màu icon riêng (ví dụ 'text-spa-brand')
  ...
  const variantStyles = {
    primary:
      "bg-spa-brand hover:bg-spa-brand-hover text-white shadow-glow-brand font-bold",
    teal: "bg-spa-sage hover:bg-spa-sage/90 text-white shadow-glow-sage font-bold",
    secondary:
      "bg-spa-bg hover:bg-spa-sage-light text-spa-dark border border-spa-border font-semibold",
    danger:
  ...
  const renderIconContent = (ic) => {
    ...
    if (
      ...
      return `<span class="inline-flex items-center justify-center shrink-0 ${activeIconColor}">${str}</span>`;
    }
    return `<i data-lucide="${str}" class="${currentIconSize} ${activeIconColor} shrink-0"></i>`;
  };
  ...
  const isEdgePosition =
    iconPosition === "start" || iconPosition === "end" || trailingIconHtml;
