...
function renderBottomNav(activeTab = 'home') {
  ...
  return `
    <nav id="mobile-bottom-nav" class="fixed left-4 right-4 z-40 max-w-sm mx-auto transition-all duration-300" style="bottom: calc(env(safe-area-inset-bottom, 16px) + 12px);">
      <div id="nav-dock" class="relative bg-white/75 dark:bg-[#1C2428]/85 backdrop-blur-xl border border-white/60 dark:border-white/10 rounded-full shadow-[0_20px_40px_-10px_rgba(0,0,0,0.15)] px-2.5 py-2 flex items-center justify-around">
        ...
        <button type="button" onclick="navigateTab('home')" id="nav-btn-home" data-tab="home" title="Trang chủ" class="relative z-10 w-12 h-12 flex items-center justify-center rounded-full text-spa-muted dark:text-white/60 hover:text-spa-brand transition-colors cursor-pointer active:scale-95">
          <i data-lucide="home" class="w-5 h-5"></i>
        </button>

        <!-- 2. TAB POS (TẠO CA / TẠO TOUR MỚI) -->
        <button type="button" onclick="navigateTab('pos')" id="nav-btn-pos" data-tab="pos" title="Tạo ca mới" class="relative z-10 w-12 h-12 flex items-center justify-center rounded-full text-spa-muted dark:text-white/60 hover:text-spa-brand transition-colors cursor-pointer active:scale-95">
          <i data-lucide="plus" class="w-5 h-5"></i>
        </button>

        <!-- 3. TAB HISTORY (LỊCH SỬ TOUR) -->
        <button type="button" onclick="navigateTab('history')" id="nav-btn-history" data-tab="history" title="Lịch sử tour" class="relative z-10 w-12 h-12 flex items-center justify-center rounded-full text-spa-muted dark:text-white/60 hover:text-spa-brand transition-colors cursor-pointer active:scale-95">
          <i data-lucide="clock" class="w-5 h-5"></i>
        </button>

        <!-- 4. TAB INCOME / REPORT (THU NHẬP / BÁO CÁO) -->
        <button type="button" onclick="navigateTab('income')" id="nav-btn-income" data-tab="income" title="${incomeLabel}" class="relative z-10 w-12 h-12 flex items-center justify-center rounded-full text-spa-muted dark:text-white/60 hover:text-spa-brand transition-colors cursor-pointer active:scale-95">
  ...
  tabs.forEach(t => {
    ...
    if (btn) {
      if (t === tabName) {
        ...
        btn.classList.remove('text-spa-muted', 'dark:text-white/60');
      } else {
        btn.classList.remove('active', 'text-white');
        btn.classList.add('text-spa-muted', 'dark:text-white/60');
      }
    }
  });
