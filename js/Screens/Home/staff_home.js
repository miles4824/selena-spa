...
function renderStaffHome(user) {
  ...
  const tourInfo = (typeof HomeService !== 'undefined') 
    ? HomeService.checkUserRunningTour(user) 
    : { isRunning: false, session: null, elapsedMin: 0, targetMin: 45 };

  const stats = (typeof HomeService !== 'undefined')
    ? HomeService.getStaffTodayStats(user)
    : { todayTours: 0, todayCommission: 0, formattedCommission: '0 đ', isMasked: true };
  ...
  const statusBadgeHtml = (typeof StatusBadge === 'function') ? StatusBadge({
  ...
  if (tourInfo.isRunning) {
    ...
    actionBtnHtml = (typeof AppButton === 'function') ? AppButton({
      text: 'VÀO XEM NGAY',
      icon: 'timer',
      iconPosition: 'left',
      variant: 'teal',
    ...
    statusDesc = (typeof getConfig === 'function') 
      ? getConfig('home_free_quote', 'Mỗi tour gội là một trải nghiệm thư giãn tuyệt vời gửi gắm đến khách hàng thân yêu.')
  ...
  const toursCardHtml = (typeof StatCard === 'function') ? StatCard({
  ...
  const commCardHtml = (typeof StatCard === 'function') ? StatCard({
...
function handleStaffTogglePrivacy() {
  ...
  if (valEl && typeof currentUser !== 'undefined') {
    ...
    valEl.innerText = isMasked ? '•••••• đ' : HomeService.formatMoney(stats.todayCommission);
  }
