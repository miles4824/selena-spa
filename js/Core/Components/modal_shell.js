...
function ModalShell({
  ...
  const titleHtml = (typeof AppTitle === 'function')
    ? AppTitle({
        text: title,
        configKey,
    ...
    : `<div class="flex items-center gap-2">${icon ? `<i data-lucide="${icon}" class="w-5 h-5 ${iconColor}"></i>` : ''}<h3 class="text-base sm:text-lg font-bold text-[#2D2424] font-serif tracking-tight">${title}</h3></div>`;
